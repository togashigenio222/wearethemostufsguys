...
const Alterador = (lista,matricula,tipodado,novodado) => {
    const AlterarDados = () => {
        ...
        if(tipodado == 'nome'){
            return indentificaroAluno.map((data) => ({...data, aluno: novodado}))
        }
        else if(tipodado == 'matricula'){
            return indentificaroAluno.map((data) => ({...data, matricula: novodado}))
        }
        else if(tipodado == 'curso'){
            return indentificaroAluno.map((data) => ({...data, curso: novodado}))
        }
        else if(tipodado == 'idade'){
            return indentificaroAluno.map((data) => ({...data, idade: novodado}))
        }
        else{
                return 'erro'
        }
...
//Alunos: Rhuan Pablo Silva Santos; José Antônio Rodrigues Santos; Daví Antonio Martins Ribeiro (TURMA 03 - E.C) 
//1
const alunos = [
    {aluno: 'Rhuan' , idade: 19, matricula: 202500035548, curso: 'EC'},
    {aluno: 'Pablo' , idade: 20, matricula: 202400045768, curso: 'SI'},
